...
def transformToMaze(alien, goals, walls, window,granularity):
    ...
    if granularity !=0:
        ...
        columns=0
        rows=0
    maze=[[[SPACE_CHAR for z in range(3)] for y in range(rows)] for x in range(columns)]
    #for every index in the maze convert to config set alien config convert back to set maze
    for i in range(columns):
        for j in range(rows):
            for k in range(3):
                w,v,shape=idxToConfig([i,j,k],[0, 0, 0],granularity, alien)
                alien.set_alien_config([w,v,shape])
                if does_alien_touch_wall(alien, walls,granularity) or not is_alien_within_window(alien, window,granularity):
                    x,y,level=configToIdx([w,v,shape], [0, 0, 0], granularity, alien)
                    maze[x][y][level]= WALL_CHAR
                elif does_alien_touch_goal(alien, goals) and maze[i][j][k]!= WALL_CHAR:
                    #if alien is touching goal
                    x,y,level=configToIdx([w,v,shape], [0, 0, 0], granularity, alien)
                    maze[x][y][level]= OBJECTIVE_CHAR
                else:
                     continue
    x,y,level=configToIdx(start, [0, 0, 0], granularity, alien)
    maze[x][y][level]= START_CHAR
    #print(maze)
    return Maze(maze,alien, granularity, offsets=[0, 0, 0], filepath=None)
                
    #instatiate a Maze oject and save to file by calling maze.saveToFile(FILENAME)
